...
func resourceNetworkACLRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(tfec2.NetworkACLEntryPropagationTimeout, func() *resource.RetryError {
		var err error

		resp, err = tfec2.FindNetworkACLEntry(conn, networkAclID, egress, ruleNumber)
